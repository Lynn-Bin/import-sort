...
import { getThirdPartyModulesRegExp } from './get-3rd-party-modules';
...
export const getSortedNodes = (
    ...
    const sortedNodesByImportOrder = order.reduce(
        (
            ...
            const x = originalNodes.filter((node) => {
                if (val === THIRD_PARTY_MODULES_SPECIAL_WORD) {
                    const thirdPartyModulesRegExp =
                        getThirdPartyModulesRegExp(importRoot);
                    let isSpecValidGroup = false;
                    if (excludeThirdModules) {
                        try {
                            var specRegExp = new RegExp(excludeThirdModules);
                            isSpecValidGroup = specRegExp.test(
                                node.source.value,
                            );
                        } catch (e) {
                            isSpecValidGroup = false;
                        }
                    }
                    if (
                        thirdPartyModulesRegExp?.full &&
                        thirdPartyModulesRegExp.full.test(node.source.value) &&
                        !isSpecValidGroup
                    ) {
                        return true;
                    }
                    if (
                        thirdPartyModulesRegExp?.left &&
                        thirdPartyModulesRegExp.left?.test(node.source.value) &&
                        !isSpecValidGroup
                    ) {
                        return true;
                    }
                    if (
                        thirdPartyModulesRegExp?.other &&
                        thirdPartyModulesRegExp.other?.test(
                            node.source.value,
                        ) &&
                        !isSpecValidGroup
                    ) {
                ...
                return node.source.value.match(new RegExp(val)) !== null;
            });
